  ...
  _createClass(Type, [{
    ...
    value: function getPhrase() {
      ...
      try {
        ...
        result = _lodash2['default'].has(global, key) ? global[key] : _lodash2['default'].has(local, key) ? local[key] : null;
      } catch (error) {
    ...
    value: function getBracket() {
      ...
      try {
        ...
        if (!_lodash2['default'].isNull(local)) local = _lodash2['default'].has(local, key) ? local[key] || local : local;
        // Check if global data exists
        global = this.router.global(keywords.global);
        // Find the phrase in the local scope
        local = _lodash2['default'].has(local, key) ? local[key] : null;
        // Find the phrase in the global scope
        global = _lodash2['default'].has(global, key) ? global[key] : null;
        ...
        if (dot) {
          ...
          if (local && !_lodash2['default'].isNull(local)) result = _lodash2['default'].has(local, key) ? local[key] : local;else if (global && !_lodash2['default'].isNull(global)) result = _lodash2['default'].has(global, key) ? global[key] : global;
        }
      } catch (error) {
